...
### **Step 6: Nail ROI Refinement + Report Build Fixes (Implemented)**
- Nail ROI refinement is extractor-only (`backend/cv/nail_features.py`); validation gate (`backend/routers/screen.py`, `NAIL_*` thresholds) untouched.
- Extractor tries MediaPipe HandLandmarker priors first (model auto-downloaded once to `backend/models/hand_landmarker.task`, singleton with graceful fallback), per-finger distal crops + OpenCV plate segmentation (aspect 0.6–1.4, solidity >0.7, size 0.5–8% frame); falls back to legacy TopHat+DoG candidates when Hands yields none.
- Boxes are not hardcoded — fully image-derived and deterministic per run.
- Contracts unchanged: `nail_rgb_v1` 21-vector, `INNER_FRACTION` 0.60, SAT 8/247, `MIN_VALID_NAILS=1`, median aggregation, green/cyan/yellow/N overlay, endpoint response + storage/base64 shapes.
- Evidence: `public/assets/test-images/good2.png` went from 5 false boxes on knuckles/background to 4 boxes on nail plates.
- Report build fixes: `TopMetricsGrid` photo-block duplication removed (photos live in `UploadedPhotosCard`), 4-tier risk scale restored; `TrendAndHealthcare` fallback object excess props removed. `tsc`/`eslint`/`next build` clean.